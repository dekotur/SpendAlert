...
- опциональный ИИ-помощник через OpenRouter;
...
Ожидаемый вывод: `Security audit passed` и `All 52 test groups passed`. Тесты
...
`/ai` сообщит об ошибке интеграции, остальное не изменится.
...
| `ai_handler.py` | Опциональный tool-loop OpenRouter и user-scoped контекст |
...
| `test_bot.py` | Автономный regression-набор из 52 групп |
...
уходят в OpenRouter. Telegram видит сообщения как транспорт. То же самое бот
объясняет пользователю через `/privacy`. Если вы держите общий инстанс — его
